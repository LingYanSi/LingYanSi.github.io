...
var Router = {
	...
	init : function(){
		...
		// 接管所有的a标签？，这样是有点不科学的，因为有些是外站链接，因此要做出限制 添加clas.router
		[].slice.call(document.querySelectorAll('.router')).forEach(function(ele){
			ele.addEventListener('click',function(event){
				event.preventDefault();
				history.pushState(null,'',this.href);
				__this.run(__this.caches);
			})
		})
	},
	error : function(){
	...
	run : function (arrS){ // 把[routerSet]数组传进来，然后遍历
		var router = location.pathname.replace('.html','');
		console.log(router)
		if(router.indexOf('/')!==0){
			location.href = "#/index" ;
			location.reload()
		...
		arrS.forEach(function(ele,index){
			...
			rsArr.forEach(function(ele,index){ // 遍历路由集合，然后做出相应处理
				var items = ( (routerSet.RouterRoot?routerSet.RouterRoot:'') +ele ).split(/\/+/); // 判断长度，然后与arr做比较，得出是否符合路由，然后执行相关函数，还要判断是不是含有id
