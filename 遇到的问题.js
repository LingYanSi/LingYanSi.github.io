	...
	主要是因为浏览器会默认的给a标签添加上样式
	因此使用 a{ color:inherit; }就可以可以了，也就是继承父类样式
	还有一点是 a标签的伪类a:hover之类的也添加上了样式
	直接使用 a{} 是无法覆盖的，因为选择器的优先级不够
	...
	上面这种写法会造成两个输入框之间有空间
	解决方案：
		1.设置他们的父元素的 font-size:0; 这说明input之间的空白会被解析成空格？
		2.float
		3.display:block;
		4.把他们连着写，删除空白。
	总结：
		其实这个空白是因为html把内联元素中间的空白字符，给编译成了一个空字符
	...
	*	   百度得知给元素加上width:0%;后就可以了，这是什么bug 
	...
	如何获取元素的可见性
	jquery $(dom).is(':hidden')
	那如何用原生js获取呢？
	1.他们的CSS display值是none。
	2.他们是type="hidden"的表单。
	3.他们的宽度和高度都显示的设置为0。
	4.一个祖元素是隐藏的或者不占据布局空间，那么此元素被认为是隐藏的的。
	...
	http://segmentfault.com/a/1190000002589171
...
----------------------------------------------------------------------------------------------
	...
	a.x = a ; // 虽说分两步执行，但是一行代码，js只会编译一遍，并且是从左往右，
			//此时的a还是原来的a,即指向{n:1}的那个a,新增x属性指向自身 b = a = { n:1 , x:{n:2} } ;
			// 其实很明白，js中的等号运算会把，等号右边的值计算完毕后再传递给左边
...
----------------------------------------------------------------------------------
	...
	NaN not a number 在parseInt(sth)的时候，有可能会返回
	NaN 是一个全局对象的属性。
	...
	在编码很少直接使用到 NaN。通常都是在计算失败时，
	作为 Math 的某个方法的返回值出现的（例如：Math.sqrt(-1)）或者尝试将一个字符串解析成数字但失败了的时候（例如：parseInt("blabla")）。
...
img标签
其实img标签获取图片的方式是 get和xmlhttprequest的get请求方式一样
因此使用img标签可以制造csrf跨站请求伪造攻击

因此数据的增删改 休要使用post请求来执行

当然，如此一来他们又可以使用form表单来攻击了，哈哈哈
