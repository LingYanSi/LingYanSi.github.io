...
/***/ function(module, exports, __webpack_require__) {
	...
	var _prev_step = __webpack_require__(10);
	...
	document.querySelector('body').onclick = function (event) {
	    if (event.target.classList.contains('del')) {
	        dispatch((0, _delete_list_item2.default)(event.target.dataset['id']));
	    }

	    if (event.target.classList.contains('history')) {
	        var id = event.target.dataset['id'];
	        dispatch((0, _prev_step2.default)(_history.appHistory[id].state));
	    }
	};
...
/***/ function(module, exports) {
	...
	var store = {
	    ...
	    fetch: '',
	    list: [{ title: '我不是潘金莲', content: '都是几个啦风格类似风格' }, { title: '大空头', content: '都是几个啦风格类似风格' }],
	    history: []
	};
